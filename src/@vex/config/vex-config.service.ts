...
export class VexConfigService {
    ...
    // add new from k_trong dev
    private _initializeConfig(): void {
        const storedConfig = localStorage.getItem('vexConfig');
        if (storedConfig) {
            const config: DeepPartial<VexConfig> = JSON.parse(storedConfig);
            this.updateConfig(config);
        } else {
            localStorage.setItem('vexConfig', JSON.stringify(this.config));
        }
    }
    ...
    updateConfig(config: DeepPartial<VexConfig>) {
        const updatedConfig = mergeDeep({ ...this._configSubject.getValue() }, config);
        this._configSubject.next(updatedConfig);
        localStorage.setItem('vexConfig', JSON.stringify(updatedConfig));
    }
